...
public:
    ...
    void inOrder(std::ostream& os, node_pointer);
    bool insert(const T&);
    bool remove(const T&);
...
private:
    ...
    int _search(const std::vector<node_pointer> &, node_pointer);
};


template <typename T>
typename BTree<T>::node_pointer BTree<T>::search(const T &t)
{
...
template <typename T>
void BTree<T>::inOrder(std::ostream& os, node_pointer node)
{
    if(node == nullptr) return;
    std::queue<node_pointer> q{};
    node_pointer p = reinterpret_cast<node_pointer>(0x001);
    q.push(node);
    q.push(p);
    while(q.size())
    {
        auto front = q.front();
        if(front == reinterpret_cast<node_pointer>(0x1))
        {
            q.pop();
            if(!q.empty()) q.push(p);
            os << std::endl;
        ...
        q.front()->dump(os);
        for(auto& c : q.front()->child)
        {
            if(c != nullptr) q.push(c);
        }
        if(q.front()->child.size() > 0) q.pop();
    }
}
...
template <typename T>
int BTree<T>::_search(const std::vector<node_pointer> &v, node_pointer node){
    if(!node) return -1;
