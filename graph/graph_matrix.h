...
struct Edge {
  ...
  int weight;  //权重
  EType type;  //边的类型
  Edge(const Te &d, int w = 1) : data(d), weight(w), type(UNDETERMINED) {}
};
...
class Graph_Matrix : public graph<Tv, Te> {
 ...
 public:
  Graph_Matrix() {  //初始化,边和顶点个数都是
    this->num_v = this->num_e = 0;
  }
  ~Graph_Matrix() {  //逐条清除边
    for (int j = 0; j < this->num_v; j++)
      for (int k = 0; k < this->num_v; k++) delete e[j][k];
  }
  ...
  virtual int insert(const Tv &thevertex) {  //插入一个顶点
    //新增一个节点:
    /* 1,
...
3, 删除该节点 */
    for (int j = 0; j < this->num_v; j++)
      if (e[i][j] != nullptr) delete e[i][j];
    for (int j = 0; j < this->num_v; j++) {
      if (e[j][i] != nullptr) delete e[j][i];
  ...
  virtual void insert(const Te &te, int i, int j,
                      int k = 1) {  //最后一个参数是权重
